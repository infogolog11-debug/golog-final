/* ============================================================
   نقطة دخول Vercel لـ /api/auth/*  (خاصة OAuth Google + Telegram)
   راجع تعليقات ملف /api/index.ts لمبرر إعادة بادئة "/api" إلى URL.
   ============================================================ */
...
function getExpressApp() {
  ...
  try {
    ...
    console.error("[vercel:api/auth] FAILED to load Express app:", err);
    throw err;
  }
}
...
export default async function handler(req: any, res: any) {
  try {
    ...
    return new Promise((resolve) => {
      app(req, res, (err?: any) => {
        if (err) {
          console.error("[vercel:api/auth] Express error:", err);
          if (!res.headersSent) {
