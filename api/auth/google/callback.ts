/* ============================================================
   نقطة دخول Vercel لـ /api/auth/google/callback (مسار محدد لـ OAuth)
   ============================================================ */
...
function getExpressApp() {
  ...
  try {
    ...
    console.error("[vercel:api/auth/google/callback] FAILED to load Express app:", err);
    throw err;
  }
}
...
export default async function handler(req: any, res: any) {
  try {
    ...
    return new Promise((resolve) => {
      app(req, res, (err?: any) => {
        if (err) {
          console.error("[vercel:api/auth/google/callback] Express error:", err);
          if (!res.headersSent) {
